...
Then(/^I should arrive at the login screen$/) do
  cal.page(LoginPage).await(message: 'Expected to arrive at login page')
end
...
And(/^I choose to get more information$/) do
  cal.page(LoginPage).more_info
end

Then(/^I am taking to the information screen$/) do
  cal.page(InfoPage).await(message: 'Expected to arrive at login page')
end

When(/^I go back from the help screen$/) do
  cal.page(InfoPage).go_back_to_login_page
end
